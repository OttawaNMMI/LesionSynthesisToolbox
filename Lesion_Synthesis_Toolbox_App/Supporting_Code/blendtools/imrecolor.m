...
%   'blursize' specifies the size of the color smoothing blur (default 5px)
...
while k<=numel(varargin);
    switch lower(varargin{k})
		...
		case 'channels'
			thisarg=lower(varargin{k+1});
			if ismember(thisarg,chanstrings)
				channels=thisarg;
			else
...
switch colormodel
	...
	case 'hsly'
		...
		A=gettfm('ypbpr');
		Ai=gettfm('ypbpr_inv');
        Iy=sum(bsxfun(@times,inpict,A(1,:,:)),3);
		switch channels
			case {'hs','color'}
				MH=M(:,:,1)/360; IH=I(:,:,1)/360;
				MH(isnan(MH))=0; IH(isnan(IH))=0;
...
if blursize>1
	Rlch=rgb2lch(imfilter(R,fk),'luv');
