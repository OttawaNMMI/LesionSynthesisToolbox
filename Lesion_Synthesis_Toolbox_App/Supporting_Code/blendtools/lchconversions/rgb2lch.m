...
%       Convert an sRGB image to the cylindrical mappings of CIELUV, CIELAB, SRLAB2, or YPbPr
%       
%   INPICT is a single RGB image of any standard image class
%   MODE is either 'luv' (default), 'lab', 'srlab', or 'ypbpr'
...
if ~strcmpi(mode,'ypbpr')
	...
	A=permute([0.299,0.587,0.114;-0.1687367,-0.331264,0.5;0.5,-0.418688,-0.081312],[1 3 2]);
	L=sum(bsxfun(@times,rgb,A(1,:,:)),3); % this is actually Y, but it has to be used in the same places as L
	Pb=sum(bsxfun(@times,rgb,A(2,:,:)),3);
	Pr=sum(bsxfun(@times,rgb,A(3,:,:)),3);
	outpict=cat(3,L,Pb,Pr);
end
