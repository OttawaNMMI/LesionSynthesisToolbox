...
%       Convert a LCH/YCH image to sRGB. LCH variant may be CIELUV, CIELAB, or SRLAB2
%       
...
if strcmpi(mode,'ypbpr')
	Ai=permute([1,0,1.402; 1,-0.3441,-0.7141; 1,1.772,0],[1 3 2]);
	R=sum(bsxfun(@times,inpict,Ai(1,:,:)),3);
	G=sum(bsxfun(@times,inpict,Ai(2,:,:)),3);
	B=sum(bsxfun(@times,inpict,Ai(3,:,:)),3);
	outpict=cat(3,R,G,B);
else
	switch thiswp
		case 'd65'
