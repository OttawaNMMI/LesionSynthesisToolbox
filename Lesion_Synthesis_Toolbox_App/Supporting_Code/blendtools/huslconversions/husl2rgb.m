...
%       'luvn' is the same as 'luvp', but constrained to the extent of RGB
%       'labn' is the same as 'labp', but constrained to the extent of RGB
...
for k=1:length(varargin);
    switch lower(varargin{k})
        ...
        case {'lab','labp','luv','luvp','luvcalc','labcalc'}
            mode=varargin{k};
        otherwise
            disp(sprintf('HUSL2RGB: unknown option %s',varargin{k}))
            return
    end
end
