...
info.datastoredir =  'M:\Perception Recons\batch_user_define_2'; 

% Auto Lesion Gen Params
d = 14; %[2]; %[8,10,12.5,15,17.5,20] 2,4,6]; %,
c = 0.5; %0.25:0.25:2.75;
...
for i = 1:length(d) 
	for j = 1:length(c) 
		...
		settings = 'C:\Users\hjuma\Documents\MATLAB\Lesion Synthesis Toolbox\lesiongenParams.mat'; 
		save(settings,'userdia'); 
		
		reconEstimate = imresize3(imgGT,[lesionBound.hdr.xdim,lesionBound.hdr.ydim,lesionBound.hdr.nplanes]); 
		disp(info.reconName)
		
		[frames,ROI_x,ROI_y,ROI_z,ROI_r] = View4D(reconEstimate,[],...
			'PixelDimensions',[lesionBound.hdr.pix_mm_xy*[1 1] lesionBound.hdr.pix_mm_z],...
